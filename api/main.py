...
async def batch_ingest_logs(
    ...
    try:
        ...
        logger.error("Failed to batch ingest logs", error=str(e))
        raise HTTPException(status_code=500, detail="Failed to batch ingest logs")
...
async def search_logs(
    ...
    try:
        ...
        async with get_db_session() as session:
            ...
            # Determine search filters from query
            time_range_start = None
            time_range_end = None
            services = None
            
            if query.filters:
                time_range_start = query.filters.time_range_start
                time_range_end = query.filters.time_range_end
...
async def analyze_root_cause(
    ...
    try:
        async with get_db_session() as session:
            ...
            error_logs_data = await log_repo.get_logs_by_time_range(
            ...
            # Fetch context logs
            context_logs_data = await log_repo.get_logs_by_time_range(
                start_time=time_start - timedelta(minutes=10),
                end_time=time_end + timedelta(minutes=10),
                services=services,
                limit=1000
            )
            ...
            error_logs = "\n".join([
                ...
                for log in error_logs_data
            ])
            
            context_logs = "\n".join([
                f"[{log.timestamp}] {log.level} {log.service_name}: {log.message}"
                for log in context_logs_data[:100]  # Limit context to avoid token limits
            ])
...
async def summarize_logs(
    ...
    try:
        async with get_db_session() as session:
            ...
            logs_data = await log_repo.get_logs_by_time_range(
                start_time=time_start,
                end_time=time_end,
                services=services,
                limit=1000
...
async def chat_with_logs(
    ...
    try:
        ...
        logger.error("Chat analysis failed", error=str(e))
